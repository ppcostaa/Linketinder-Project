...
class LocalizacaoRepositoryTeste extends Specification {
    ...
    def "deve salvar uma localizacao com sucesso"() {
        ...
        when: "tenta salvar a localizacao"
        def resultado = repository.salvarLocalizacao(localizacao)

        then: "a localizacao é salva com ID gerado"
        resultado.localizacaoId == 123
        resultado.cep == "12345-678"
    ...
    def "deve lançar exceção e fazer rollback ao falhar ao salvar localizacao"() {
        given: "uma localizacao válida"
        def localizacao = new Localizacao(cep: "12345-678", pais: "Brasil")
        ...
        when: "tenta salvar a localizacao"
        repository.salvarLocalizacao(localizacao)
    ...
    def "deve listar todas as localizacoes com sucesso"() {
        ...
        resultSet.getInt("ID_LOCALIZACAO") >>> [1, 2]
        resultSet.getString("CEP") >>> ["12345-678", "98765-432"]
        resultSet.getString("PAIS") >>> ["Brasil", "Argentina"]

        when: "lista todas as localizacoes"
        def localizacoes = repository.listarLocalizacoes()

        then: "retorna lista com todas localizacoes"
        localizacoes.size() == 2
        localizacoes[0].localizacaoId == 1
        localizacoes[0].cep == "12345-678"
        localizacoes[0].pais == "Brasil"
        localizacoes[1].localizacaoId == 2
        localizacoes[1].cep == "98765-432"
        localizacoes[1].pais == "Argentina"
    }
